...
> **这份文档和 README 的分工**
>
> - `README.md` 按「目录 / 模块」**纵向**组织：这里有什么文件、每个模块提供什么接口、怎么用。
> - 本文按「功能」与「技术机制」**横向**贯通：一个能力是怎样从界面一路贯穿到存储的；
...
- 从 `RunEnvironment` 取父级的 `ModeContext`，**照搬**提示词 / 技能 / 记忆开关 / 确认名单，
  只把 `SUBAGENT_EXCLUDED`（`spawn_agent`/`ask_user`/`submit_plan`/`todo_write`）剔掉。
...
**这个机制是在实测中被逼出来的**：一次 `spawn_agent` 跑了整 360 秒没往外播任何东西，
...
| `subagentEvents` | 子代理实时动静 | `spawn_agent` 那步到达就清 |
