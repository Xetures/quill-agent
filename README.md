...
| `outline` | 代码 | 列出文件（或目录）的符号骨架：类 / 函数 + 行号。**读长文件前先看它**（见 7.28） |
| `find_symbol` | 代码 | 跨文件找符号的定义与引用位置。正则启发式，比 `search_content` 准（见 7.28） |
| `git_status` | 版本控制 | 看当前分支、已暂存 / 未暂存的改动、未跟踪的文件（见 7.29） |
| `git_diff` | 版本控制 | 看尚未提交的改动内容（unified diff），可限定单个文件（见 7.29） |
| `git_log` | 版本控制 | 看最近的提交记录（短哈希 + 日期 + 说明） |
| `git_commit` | 版本控制 | 提交到**本地**仓库：不可逆，每次先问用户；**不 push**（见 7.29） |
...
> `mcp__<服务器>__<工具>`，分类是 `MCP · <服务器>`）。它们连上之后才注册 —— 见 7.32。
...
### 7.29 git 工具：为什么不把 `git` 交给 `run_command`
