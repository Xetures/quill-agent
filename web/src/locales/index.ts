 ...
 * 界面语言。
 *
 * **界面文案已全部接入**：两档语言的键一一对应（`make i18n` 看剩余量），加文案时
 * 两边一起加。
 *
 * **内置提示词不在这一套里**：它们是 `prompt/` 下的 Markdown，只有一份中文，且是给
 * 用户自己编辑的。所以切到英文时**界面是英文、提示词仍是中文** —— 这一点在语言选项的
 * 说明里如实写了。宁可说清楚，也不要让人切过去以后怀疑切换没生效。
 *
