...
给要改它的人看的详细文档，按「目录 / 模块」纵向组织：每一层为什么这么设计、一个行为落在哪个
文件里、二次开发从哪儿下手。

- 只想把它跑起来用一用 → [README.md](./README.md)（精简版）
- 想看一个能力从界面一路贯穿到存储的完整链路 → [ARCHITECTURE.md](./ARCHITECTURE.md)（横向）
...
- [6. 界面层：Vue](#6-界面层vue)
  ...
  - [6.38 字号与语言：都照主题那套来](#638-字号与语言都照主题那套来)
- [许可](#许可)
...
@registry.tool(
    ...
    category="files",  # 分类 key：只用于界面筛选，不发给模型；中文由界面翻译（见 6.38）
...
后来偏好设置拆成了几节（显示 / 对话 / 联网搜索 / MCP / 归档 / 关于），这一节跟着归了
进去：都是「配一次、用很久」的设置，一级导航留给模式、工具这类**天天要切**的东西。
...
显示相关的三样（明暗、字号、语言）同属「偏好设置 → 显示」这一页。前两样共用一套做法：
**往 `<html>` 上挂一个属性，其余全交给 CSS 变量** —— 组件里不写任何「如果是大字号就……」
的判断，加一档只是加一组变量。

**字号**分小 / 标准 / 大三档，标准档的倍数就是 1 —— 也就是这套 token 引入之前的样子，
取值逐字相同，所以这层改造本身不改变任何观感。实现分两步：

1. 所有硬编码的 `font-size: 12px` 换成 token（`--fs-2xs` 到 `--fs-xl` 六个，取值原样保留，
   纯替换，共 92 处 / 25 个文件）；
2. token 定义改成 `calc(12px * var(--font-scale))`，`html[data-font-size='large']` 只改那个
   倍数。

**只缩放字号、不动间距与高度**，这点是刻意的：间距承担的是几何 —— 四块玻璃板的缝、桌面壳
里让给系统标题栏的那 28pt 都按 pt 对齐系统，跟着一起放大就不再是「字大了」，而是「界面被
整体拉过一遍」。代价值得说清：大字号下个别固定高度的行会挤。实测全文只有一处 18px 的固定
高度，而且它是**分隔线**不是文字框，所以没有要改的。

Element Plus 的字号接同一套 token（`--el-font-size-*`）—— 它那些用 `size` 属性的组件
（表格、按钮、下拉）只改我们自己的 token 是管不到的。

**语言的骨架已经就位，但文案迁移是分期的：**

- 已接入：侧边栏导航、偏好设置整页（含「显示」页自己）、归档确认框、以及 Element Plus
  自己的文案（表格空态、确认框按钮 —— 换语言包即可）；
- 尚未迁移：其余页面的模板文案。`en-US` 那一档的说明里如实写了「界面文案迁移中，部分
  位置仍是中文」—— 宁可说清楚，也不要让人切过去以后怀疑切换没生效。
- 迁移是**一页一提交**，进度看 `make i18n`（`scripts/i18n_progress.py` 数模板里还剩多少
  中文，按文件排序，好决定下一批挑哪一页）。它刻意不做 CI 门禁：数量归零前它一直在报数，
  卡在 CI 里只会变成噪音。

后端那边有一处配套改动值得记：**工具分类不再是中文，而是稳定 key**（`files` / `shell` /
`mcp:<服务器名>`）。`category` 本来就只用于界面筛选、不会发给模型，所以它属于界面文案 ——
显示名与排序都在 `web/src/utils/tool-category.ts` 里定（排序也放前端：按重要性排比按 key
的字母排好用，而且换语言不会乱）。

key 用「区域.用途」（`settings.sections.display`）而不是中文原文：拿中文当 key，润色一次
文案就得连带改一遍引用，而且改完再也搜不到这句话还在哪儿用过。

**语言存 localStorage，同时往后端偏好写一份**（键名 `language`）：首屏要立刻用上，等不了
后端那一趟往返（首屏那次由 `index.html` 的内联脚本负责）；写后端是因为后端也有要显示给
用户的文案（工具分类这类），将来它得知道自己该用哪种语言。本地没存过时采纳后端那份 ——
换了浏览器或清了站点数据，不该退回中文。

最后划一条边界：**提示词、工具描述、技能正文不参与翻译**。它们会进模型上下文，属于内容
而不是界面（用户自己能改 `prompt/*.md`）。切语言不该改变模型说的话。
