...
**第一次打开要多一步。** 这个包**没有签名**，macOS 会拦一下（Gatekeeper）：

1. 访达 → **Applications**，找到 Quill；
2. **右键点它 → 打开**（不是双击）；
3. 弹窗点「打开」。

之后就与普通 App 一样，双击即可。万一右键那一步没出现，去
**系统设置 → 隐私与安全性** 最下方，那条「Quill 已阻止使用」旁边点「仍要打开」。
命令行也行，效果相同：
...
> **为什么不签名**：Apple 每年收 99 美元，而且**只签名不公证照样被拦** —— 要过就得
> 签名 + 公证两件都做，还得给包里的 Python 运行时逐个 .so 签到位。1.0 之前没做，
> 等真有人需要「下载即用」再说。代价就是上面这三下。

卸载：把 `/Applications/Quill.app` 拖去废纸篓。**数据不会跟着删**
（在 `~/Library/Application Support/Quill/`，见下面的「数据放在哪」），要清干净再手动
删那个目录。
...
| **API设置** | 模型连接 |
| **偏好设置** | 显示（明暗 / 字号 / 语言）、对话、联网搜索、MCP、归档、关于 |
